 ...
 */
(function(f,p,k){var g=f.location,s=g.protocol+"//"+g.hostname+(g.port!==""?":"+g.port:""),e="__",a=["html","json","jsonp","script","style","text","xml"],r,q;function j(i){return i;}function v(n,i){return i;}function m(n,i){return(i||"")+n;}function l(i){if(!(this instanceof l)){return new l(i);}this.cfgs=[];this.setup(i);}l.prototype={setup:function(i){this.cfg=i||{};return this;},sajax:function(C){var A=this.cfgs,B=A.length,x=A[B-1],E=C.dataType,w=C.url.indexOf("http")===0&&C.url.indexOf(s)===-1,z=E==="script",G=E==="style",i=B>0?w&&(z||G)?true:C.defer:false,F=C.success||(z||G?v:j),D=C.error||k.noop,n=C.complete||k.noop;A.push(k.extend(true,C,this.cfg,C,{sendAfterSend:[],dataType:(!w&&z||G)?"text":E,success:function(I,H){if(!x||e in x){if(z&&!w){k.globalEval(I);}else{if(G&&!w){I=k.styleEval(I);}}C.status=w?"success":H;C[e]=F.call(C,I,x&&x[e],A[B+1],C.status);if(C.nextSuccess){C.nextSuccess();}else{if(C.sendAfterSuccess){C.sendAfterSuccess();}}}else{x.nextSuccess=k.proxy(function(){C.success(I,H);},C);}},error:function(J,H,I){D.call(C,J,H,I);},complete:function(I,H){n.call(C,I,H);}}));function y(){var H,I;if(G&&w){k.getCSS(C.url,C.success);}else{k.ajax(C);}if(C.sendAfterSend.length>0){for(H=0,I=C.sendAfterSend.length;H<I;++H){C.sendAfterSend[H]();}}}if(i){x.sendAfterSuccess=y;this.lastDefer=C;}else{if(this.lastDefer){this.lastDefer.sendAfterSend.push(y);}else{y();}}return this;},bundle:function(){var w=arguments,y=k.isFunction(w[w.length-1])?Array.prototype.pop.call(w):v,x,z;for(x=0,z=w.length-1;x<z;++x){this.text(w[x],m);}return this.text(w[x],function(B,A,n,i){var C=m(B,A);k.globalEval(C);return y(C,A,n,i);});}};function o(i){l.prototype[i]=function(n,x,w){if(typeof n==="string"){if(typeof x!=="boolean"){w=x;x=false;}n={url:n,defer:x,success:w};}n.dataType=i;return this.sajax(n);};}for(r=0,q=a.length;r<q;++r){o(a[r]);}l.prototype.js=l.prototype.script;l.prototype.css=l.prototype.style;function c(i){l[i]=function(){return l.prototype[i].apply(new l(),arguments);};}for(r in l.prototype){c(r);}f.Sexy=k.sajax=l;
/*
