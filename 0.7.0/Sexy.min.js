 ...
 */
(function(d){var m=location.protocol+"//"+location.hostname+(location.port!==""?":"+location.port:""),i="send-",b="success-",h="error-",j="script",k="style",l="text",e="__",n=/\S/,c;function a(p){if(!(this instanceof a)){return new a(p);}this.cfgs=[];this.setup(p);this.evt=d(this);}c=a.prototype={setup:function(p){this.cfg=p;return this;},bundle:function(){var p=arguments,r=d.isFunction(p[p.length-1])?Array.prototype.pop.call(p):o,q,s;for(q=0,s=p.length-1;q<s;++q){this.text(p[q],g);}return this.text(p[q],function(v,t,u){var w=g(v,t,u);d.globalEval(w);return r(w,t);});}};d.each(["html","json","jsonp",j,k,l,"xml"],function(p,q){c[q]=function(r,A){var B=this.evt,w=this.cfgs,x=w.length,u=x-1,y=(typeof r!=="string")?r:{url:r},t=q===j,D=q===k,v=y.url.indexOf("http")===0&&y.url.indexOf(m)===-1,C=r.success||A||(t||D?o:f),z=r.error||function(){};w.push(y=d.extend(true,{},this.cfg,y,{dataType:((t||D)&&!v)?l:q,success:function(G,E){var F=w[u];if(!F||e in F){if(G&&G.target===B[0]){E=G.data[1];G=G.data[0];}if(t){d.globalEval(G);}else{if(D){if(G&&n.test(G)){d("body").append('<style type="text/css">'+G+"</style>");}}}y[e]=C.call(y,G,E,F&&F[e]);B.trigger(b+x);}else{B.bind(b+u,[G,E],y.success);}},error:function(){z.apply(y,arguments);B.trigger(h+x,arguments);}}));function s(){if(v&&D){d.getCSS(y.url,y.success);}else{d.ajax(y);}B.trigger(i+x);}if(v&&(t||D)&&x>0){B.bind(b+u,s);this.defer=x;}else{if(this.defer){B.bind(i+this.defer,s);}else{s();}}B.bind(h+u,function(F,H,E,G){y.error.call(y,H,E,G);});return this;};});c.js=c[j];c.css=c[k];d.each(c,function(p,q){a[p]=function(){return q.apply(new a(),arguments);};});function f(p){return p;}function o(r,p,q){return q;}function g(r,p,q){return(q||"")+r;}d.sajax=window.Sexy=a;})(jQuery);
